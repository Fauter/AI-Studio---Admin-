...
const lines = fs.readFileSync('C:/Users/lmfau/OneDrive/Escritorio/Code/ZZZ/AI Studio Admin/pages/CashClosuresPage.tsx', 'utf8').split('\n');
const out = [];
...
for (let i = 0; i < lines.length; i++) {
    ...
    if (line.includes("import {") && line.includes("} from 'lucide-react';")) {
        out.push(line.replace("} from 'lucide-react';", ", ReceiptText, Plus, X } from 'lucide-react';"));
        continue;
    }

    if (line.includes("import React, { useState, useEffect, useMemo } from 'react';")) {
        out.push(line);
        out.push("import { createPortal } from 'react-dom';");
        out.push("import { useAuth } from '../hooks/useAuth';");
        out.push("import { formatDateTime24h } from '../lib/dateFormatters';");
        continue;
    }

    if (line.includes("interface PartialClose {")) {
        out.push("type PartialMovementType = 'withdrawal' | 'expense';\n");
        out.push(line);
        continue;
    }

    if (line.includes("notes: string | null;")) {
        out.push(line);
        out.push("    movement_type: PartialMovementType | null;");
        continue;
    }

    if (line.includes("type TabKey = 'computo' | 'shifts' | 'partials' | 'history';")) {
        out.push("type TabKey = 'computo' | 'shifts' | 'partials' | 'expenses' | 'history';");
        continue;
    }

    if (line.includes("function formatDate(dateStr: string) {")) {
        out.push(`function normalizeMovementType(value: string | null | undefined): PartialMovementType {
...
const formatDate = formatDateTime24h;`);
    ...
    if (line.includes("const { garageId } = useParams<{ garageId: string }>();")) {
        out.push(line);
        out.push("");
        out.push("    const [loading, setLoading] = useState(true);");
        out.push("    const { profile } = useAuth();");
        out.push("    ");
        out.push("    const [isExpenseModalOpen, setIsExpenseModalOpen] = useState(false);");
        out.push("    const [expenseAmount, setExpenseAmount] = useState('');");
        out.push("    const [expenseRecipient, setExpenseRecipient] = useState('');");
        out.push("    const [expenseNotes, setExpenseNotes] = useState('');");
        out.push("    const [isCreatingExpense, setIsCreatingExpense] = useState(false);");
        out.push("    const [expenseFormError, setExpenseFormError] = useState<string | null>(null);");
        out.push("");
        out.push("    useEffect(() => {");
        out.push("        if (isExpenseModalOpen) {");
        out.push("            document.body.style.overflow = 'hidden';");
        out.push("        } else {");
        out.push("            document.body.style.overflow = '';");
        out.push("        }");
        out.push("        return () => {");
        out.push("            document.body.style.overflow = '';");
        out.push("        };");
        out.push("    }, [isExpenseModalOpen]);");
        // skip the existing setLoading line
        while (!lines[i+1].includes("const [withdrawingId")) i++;
        continue;
    }

    if (line.includes("const toggleExpandHistory = (id: string) => {")) {
        // output the original
        out.push(line);
        while (lines[i+1] !== "    };") {
            i++;
    ...
    if (line.includes("const pendingPartials = useMemo(() =>")) {
        out.push(`    const pendingPartials = useMemo(() =>
    ...
    if (line.includes("const partials: UnifiedWithdrawn[] = partialCloses")) {
        inPartials = true;
        out.push(`        const partials: UnifiedWithdrawn[] = partialCloses
    ...
    if (line.includes("const tabs: { key: TabKey; label: string; icon: React.ElementType; count?: number }[] = [")) {
        out.push(`    const tabs: { key: TabKey; label: string; icon: React.ElementType; count?: number; customActiveColors?: string; customBadgeColors?: string; }[] = [
    ...
    if (line.includes("{tabs.map(tab => {")) {
        out.push(`                        {tabs.map(tab => {
                            const Icon = tab.icon;
        ...
        while (lines[i+1] !== "                </div>") i++;
        i++;
        continue;
    }

    if (line === "        </div>" && lines[i+1] === "    );" && lines[i+2] === "}") {
        out.push(`                {activeTab === 'expenses' && (
...
fs.writeFileSync('C:/Users/lmfau/OneDrive/Escritorio/Code/ZZZ/AI Studio Admin/pages/CashClosuresPage.tsx', out.join('\\n'));
console.log('Patched CashClosuresPage.tsx cleanly via line reader');
