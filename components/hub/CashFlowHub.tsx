...
export default function CashFlowHub({ garages }: CashFlowHubProps) {
    ...
    const activeGarageIds = useMemo(() => {
        if (selectedGarageId === 'all') return garages.map(g => g.id);
        return [selectedGarageId];
    }, [garages, selectedGarageId]);

    const { data: peakHoursData, labels: peakHoursLabels, loadingPeakStays } = usePeakStays(
        activeGarageIds,
        peakPeriod,
        historicalChartView,
        peakMode
